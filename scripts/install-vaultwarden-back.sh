...
# echo "拉取镜像..."
# docker pull ttionya/vaultwarden-back

`dirname $0`/create-dir.sh $base_data_dir/vaultwarden-back
...
`dirname $0`/stop-container.sh vaultwarden-back
docker run -d \
--restart=always \
-m 50M \
--name vaultwarden-back \
-v $base_data_dir/vaultwarden-back/config:/config \
